...
def mean_std(df, columns=None):
    """
    Calculate the mean and standard deviation for each specified column 
    in a DataFrame.

    Parameters:
    df (pd.DataFrame): The DataFrame with measurements.
    columns (list, optional): List of columns to perform calculations on. If None, uses all columns.

    Returns:
    pd.DataFrame: DataFrame containing means and standard deviations for each specified column.
    """
